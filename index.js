...
const app = express();
const port = 4444;
const ws = new WebSocket(`ws://${process.env.HOMEBRIDGE_IP_ADDRESS}/`);
...
app.get("/api/play", async (req, res) => {
  const { stdout } = await exec(
    `atvscript --id ${process.env.APPLETVMAC} --airplay-credentials ${process.env.APPLETVTOKEN} play`
  );
...
app.get("/api/pause", async (req, res) => {
  const { stdout } = await exec(
    `atvscript --id ${process.env.APPLETVMAC} --airplay-credentials ${process.env.APPLETVTOKEN} pause`
  );
  stdout.on("data", function (data) {
    console.log(process.stdout.write(data));
...
app.get("/api/playing", async (req, res) => {
  const { stdout } = await exec(
    `atvscript --id ${process.env.APPLETVMAC} --airplay-credentials ${process.env.APPLETVTOKEN} playing`
  );
...
const updates = async () => {
  const { stdout } = await exec(
    `atvscript --id ${process.env.APPLETVMAC} --airplay-credentials ${process.env.APPLETVTOKEN} push_updates`
  );
  ...
  stdout.on("data", function (data) {
    ...
    if (parsedData.power_state === "on") {
      ws.send(
        JSON.stringify({
          topic: "set",
          payload: { name: "isAppleTVOn", characteristic: "On", value: true },
        })
      );
    }
    if (parsedData.power_state === "off") {
      ws.send(
        JSON.stringify({
          topic: "set",
          payload: { name: "isAppleTVOn", characteristic: "On", value: false },
        })
      );
    }
